...
Candidate validation is version-driven: CI derives the active `versionName`/`versionCode` from the Android build and validates the matching `apps/pose-studio/releases/pose-studio-v<semver>.md` manifest instead of hard-coding the currently published beta version into workflow contracts. The candidate workflow is triggered only by a successful Pose Studio workflow on `main`; the publisher still verifies the required exact-SHA `CI`, `Pose Studio`, and `CI Contracts` runs before it can freeze a new candidate.

The installable commercial-beta provenance remains pinned by `apps/pose-studio/releases/commercial-beta.lock.env`. The lock records the exact tag, candidate SHA, manifest digest, Android version, debug-keystore digest, certificate digest and expected APK filename. After the locked `pose-studio-v0.2.1` prerelease became immutable and complete, the automatic commercial-beta publication workflow is retired so unrelated `main` changes cannot create redundant release runs. The lock and matching release manifest remain repository evidence and must stay internally consistent. A future commercial-beta line requires an explicit new publication design rather than reactivating the historical one implicitly.
